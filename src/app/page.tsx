    ...
    <div className="min-h-screen bg-[#F4F7FA] text-slate-900 font-sans pb-20">
      ...
      <main className="max-w-6xl mx-auto p-6 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <section className="bg-gradient-to-br from-indigo-600 to-blue-700 p-6 rounded-3xl shadow-lg text-white">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
              <Gamepad2 /> Piloto Tecnologías RV
            </h3>
            <div className="bg-white/10 p-4 rounded-2xl border border-white/20">
              <h4 className="font-bold text-sm">Entorno: Exergames Inmersivos</h4>
              <p className="text-xs text-blue-100 mt-1">Simulación de marcha con estímulos multisensoriales para potenciar neuroplasticidad.</p>
              <button className="mt-4 w-full py-2 bg-white text-blue-700 rounded-xl font-bold text-xs hover:bg-blue-50 transition-colors">
        ...
        <section className="bg-slate-900 text-white rounded-[40px] p-8 relative overflow-hidden">
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            <h2 className="text-2xl font-bold flex items-center gap-3"><Layers className="text-blue-400" /> Monitoreo de Sesión</h2>
            <div className="bg-white/10 px-6 py-3 rounded-2xl border border-white/20 flex items-center gap-6">
              <span className="text-3xl font-mono font-bold text-blue-400">{formatearTiempo(segundos)}</span>
              <div className="flex gap-2">
                <button onClick={() => setTimerActivo(!timerActivo)} className="p-2 bg-blue-600 rounded-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className={`p-4 rounded-2xl border ${segundos > 2400 ? 'bg-blue-600 border-white' : 'bg-white/5 border-white/10 opacity-50'}`}>
              <h4 className="font-bold text-xs uppercase">1. Calentamiento (5 min)</h4>
              <p className="text-[10px] text-blue-100">Movilidad y respiración consciente.</p>
            </div>
            <div className={`p-4 rounded-2xl border ${segundos <= 2400 && segundos > 600 ? 'bg-blue-600 border-white' : 'bg-white/5 border-white/10 opacity-50'}`}>
              <h4 className="font-bold text-xs uppercase">2. Intervención RV (30 min)</h4>
              <p className="text-[10px] text-blue-100">Estímulos multisensoriales.</p>
            </div>
            <div className={`p-4 rounded-2xl border ${segundos <= 600 && segundos > 0 ? 'bg-blue-600 border-white' : 'bg-white/5 border-white/10 opacity-50'}`}>
              <h4 className="font-bold text-xs uppercase">3. Enfriamiento (10 min)</h4>
              <p className="text-[10px] text-blue-100">Estiramientos y feedback.</p>
            </div>
          </div>
        </section>
        ...
        <section>
          <h2 className="text-xl font-bold mb-6 flex items-center gap-2"><ClipboardCheck className="text-blue-600" /> Batería de Evaluación Clínica</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.keys(detallesPruebas).map((test) => (
              <button 
                key={test}
                onClick={() => setPruebaSeleccionada(test)}
                className={`p-6 rounded-2xl border-2 font-black text-xl transition-all ${pruebaSeleccionada === test ? 'border-blue-600 bg-blue-50 text-blue-600' : 'bg-white border-slate-100 text-slate-400 hover:border-blue-200'}`}
              >
                {test}
              </button>
            ))}
          ...
          {pruebaSeleccionada && (
            <div className="mt-6 p-6 bg-white border-l-8 border-blue-600 rounded-2xl shadow-lg animate-in fade-in slide-in-from-left-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg">Herramienta: {pruebaSeleccionada}</h3>
                <button onClick={() => setPruebaSeleccionada(null)} className="text-slate-300 hover:text-red-500"><X /></button>
              </div>
              <div className="grid md:grid-cols-2 gap-4 text-sm">
                <div><h4 className="text-blue-600 font-bold uppercase text-[10px] mb-1">Descripción</h4><p>{detallesPruebas[pruebaSeleccionada].desc}</p></div>
                <div><h4 className="text-slate-400 font-bold uppercase text-[10px] mb-1">Objetivo</h4><p>{detallesPruebas[pruebaSeleccionada].obj}</p></div>
              </div>
            </div>
          )}
        </section>
